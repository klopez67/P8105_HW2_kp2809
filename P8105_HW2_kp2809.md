...
- check for completeness and correctness across datasets (e.g. by
  viewing individual datasets and using anti_join) –\> describe how many
  distinct names in each and check if the highest is still present in
  the end
...
```
    ...
    ## # A tibble: 1,144 × 7
    ##    baker     series episode technical result signature_bake         show_stopper
    ##    <chr>      <dbl>   <dbl>     <dbl> <chr>  <chr>                  <chr>       
    ##  1 annetha        1       1         2 IN     "Light Jamaican Black… Red, White …
    ##  2 david          1       1         3 IN     "Chocolate Orange Cak… Black Fores…
    ##  3 edd            1       1         1 IN     "Caramel Cinnamon and… N/A         
    ##  4 jasminder      1       1        NA IN     "Fresh Mango and Pass… N/A         
    ##  5 jonathan       1       1         9 IN     "Carrot Cake with Lim… Three Tiere…
    ##  6 louise         1       1        NA IN     "Carrot and Orange Ca… Never Fail …
    ##  7 miranda        1       1         8 IN     "Triple Layered Brown… Three Tiere…
    ##  8 ruth           1       1        NA IN     "Three Tiered Lemon D… Classic Cho…
    ##  9 lea            1       1        10 OUT    "Cranberry and Pistac… Raspberries…
    ## 10 mark           1       1        NA OUT    "Sticky Marmalade Tea… Heart-shape…
    ## # ℹ 1,134 more rows
...
Describe the cleaning process any questions or state why you made those
decisions. Breifly discuss the final dataset I imported the datasets
accounting for any missing data. I skipped 2 lines for the “results_df”
since the first two rows were not column names. I cleaned the column
names by using janitor and clean names functions. I also seperated the
bakers names from the “bakers_df” as it had both first and last names.
This made it easier to combine these datasets by two identifiers series,
episode, and name. I combined bakers_df by only bakers first name and
series as it did not have a episode column. Since each datasets measured
different observations, I joined the datasets. I used full join to
ensure that no episodes were lost from left or right joining when
joining results_df and bakes_df. I then joined the bakers_df by baker
and series to the dataset I then The final dataset has 1144
